...
파일역할index.html웹진의 전체 틀(레이아웃)style.css디자인, 색상, 글꼴 등 꾸미기main.js데이터를 읽어서 화면에 표시하는 프로그램
...
예시
파일명의미fig_202512_1.png2025년 12월호, 첫 번째 논문 그림fig_202512_2.png2025년 12월호, 두 번째 논문 그림fig_202601_1.png2026년 1월호, 첫 번째 논문 그림
...
GitHub 저장소에서 assets/images/ 폴더를 클릭합니다.
우측 상단 Add file → Upload files 를 클릭합니다.
규칙에 맞게 이름을 지은 이미지 파일(예: fig_202602_1.png)을 끌어다 놓습니다.
아래쪽 Commit changes 버튼을 눌러 저장합니다.


2단계 — data.json 수정하기
data.json 파일이 현재 웹진에 표시되는 내용의 전부입니다.

GitHub에서 data.json 파일을 클릭합니다.
우측 상단 연필 아이콘(✏️ Edit) 을 클릭합니다.
아래 구조를 참고하여 내용을 수정합니다.
...
각 항목 설명
항목설명vol / issue권호 정보date발간 연월editorsNote에디터 노트 본문badges논문 유형 배지. original-article, review-article, case-report 등yearInfo논문 출판 연도title논문 제목author저자명abstract초록pearl핵심 요약 한 줄 (PEARL 박스에 표시됨)doiLink논문 DOI 링크image그림 파일 경로 (assets/images/파일명)caption그림 아래 설명reversetrue로 하면 그림이 왼쪽, 텍스트가 오른쪽으로 배치됨
...
GitHub에서 data.json을 열고 전체 내용을 복사합니다 (Ctrl+A → Ctrl+C)
...
③ 그 다음 data.json을 새 호 내용으로 업데이트합니다
...
매 호 주소를 체계적으로 기록하고 싶다면 GitHub Wiki를 활용하세요. 코딩 없이 텍스트로 목록을 관리할 수 있습니다.
Wiki 시작하기

저장소 상단 탭에서 Wiki 를 클릭합니다.
Create the first page 를 클릭합니다.
아래 예시처럼 호수별 주소를 기록합니다.
Save page 를 눌러 저장합니다.
...
새 호 발간 전 아래 순서대로 진행하세요.
 ...
 현재 data.json을 archives/YYYY-MM.json으로 복사해 저장했는가?
 data.json을 새 호 내용으로 업데이트했는가?
 웹사이트에서 정상적으로 표시되는지 확인했는가?
 GitHub Wiki에 새 호 주소를 추가했는가?


❓ 문제가 생겼을 때
증상원인 및 해결웹진이 텅 비어 있음data.json 문법 오류. 쉼표(,)나 따옴표(")가 빠진 곳이 있는지 확인이미지가 안 보임image 경로가 실제 파일명과 다른지 확인 (대소문자 포함)과거 호 링크가 안 열림archives/ 폴더에 해당 JSON 파일이 있는지 확인수정 후 반영이 안 됨GitHub Pages 배포는 최대 2분 소요. 기다린 후 새로고침
